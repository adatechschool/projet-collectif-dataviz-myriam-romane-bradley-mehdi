async function generateKey () {
  ...
  async function execute () {
    if(document.getElementById("ChartFrameworks")) {
      document.getElementById("ChartFrameworks").remove();
    }
    let token = await generateKey();
      ...
      let frameworks = {
          ...
          symphony : ["symphony","Symphony","SYMPHONY","Symfony","SYMFONY","symfony"],
      }
      let nbOffresTrouvees=0;
      let nbOffresParFramework=[];
      for (const framework in frameworks) {
          let counter = 0;
          for (let i =0; i<finalArray.length; i++) {
              let string = JSON.stringify(finalArray[i]);
              let search=0;
              frameworks[framework].forEach(element => { 
          ...
          if (counter >= 0) {
              ...
              console.log(`nombre d'offres ${framework} : ${counter}`);
              nbOffresParFramework.push(counter);
          }
      }
      console.log(nbOffresTrouvees);
  
      var data = [{
            data: nbOffresParFramework,
            backgroundColor: [
              "#264653",
              "#2a9d8f",
              "#e9c46a",
          ...
          var options = {
            title: {
              display: true,
              text: "Répartition par framework"
            },
            legend: {
              position: 'bottom',
            },
          ...
          function addElement() {
            ...
            // add the attributes
            newCanvas.setAttribute("id","ChartFrameworks");
            newCanvas.setAttribute("width","300");
            newCanvas.setAttribute("height","300");
        
            //ajouter dans le HTML
            document.getElementById("chart4").appendChild(newCanvas);
          }
